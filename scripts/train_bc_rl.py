...
from IGDrivSim.model import make_train
from IGDrivSim.dataset import (
    N_TRAINING_INTER,
    N_TRAINING,
    N_TRAINING_INTER_SPEED_0_5,
    N_TRAINING_INTER_SPEED_0_7,
)
...
parser.add_argument(
    "--loss_weight_bc", "-w_bc", type=float, help="Weight of the BC loss"
)
parser.add_argument(
    "--loss_weight_rl", "-w_rl", type=float, help="Weight of the RL loss"
)
...
if __name__ == "__main__":
    ...
    if config["filter_fun_name"]:
        ...
        elif (config["filter_fun_name"] == "inter_speed_filter_fun") and (
            config["filter_fun_args"]["min_mean_speed"] == 0.5
        ):
            config["num_training_data"] = N_TRAINING_INTER_SPEED_0_5
        elif (config["filter_fun_name"] == "inter_speed_filter_fun") and (
            config["filter_fun_args"]["min_mean_speed"] == 0.7
        ):
            config["num_training_data"] = N_TRAINING_INTER_SPEED_0_7
        else:
            print(
                "Counting training data that satisfy the filter (this may take some time)"
            )
            config["num_training_data"] = count_unfiltered_data(
                config["filter_fun_name"], config["filter_fun_args"]
            )
    else:
