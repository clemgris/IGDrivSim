...
class make_train:
    def __init__(
        ...
        # Minibatch
        self.n_minibatch = max(
            [
                n
                for n in range(len(self.devices), 0, -1)
                if self.config["num_envs"] % n == 0
            ]
        )
    ...
    def train(
        ...
        init_rnn_state_train = ScannedRNN.initialize_carry(
            (self.mini_batch_size, feature_extractor_shape)
        )
        ...
        if self.config["lr_scheduler"] == "linear":
            n_update_per_epoch = (
                self.config["num_training_data"] * self.config["num_files"] / N_FILES
            ) // self.config["num_envs"]
            transition_step = n_update_per_epoch * self.config["lr_transition_epoch"]
            learning_rate = optax.linear_schedule(
                self.config["lr_max"], 0.0, transition_step
            )
            ...
            learning_rate = optax.cosine_decay_schedule(
                self.config["lr_max"], transition_step
            )
        ...
        init_rnn_state_eval = ScannedRNN.initialize_carry(
            (self.config["num_envs_eval"], feature_extractor_shape)
        )
        ...
        def _log_step(cary, rng_extract):
            ...
            transition = TransitionBC(done, None, obsv)
        ...
        def _env_step_expert(cary, rng_extract):
            ...
            transition = TransitionBC(done, expert_action, obsv)
        ...
        def _env_step_imitator(cary, rng_extract):
            ...
            rnn_state, action_dist, value, _, _, _ = network.apply(
                params, rnn_state, (jax.tree_map(extand, obsv), done[jnp.newaxis, ...])
            )
            ...
            action = datatypes.Action(
                data=action_data,
                valid=jnp.ones((self.mini_batch_size, 1), dtype="bool"),
            )
            ...
            transitionRL = TransitionRL(done, action, value, reward, obsv, log_prob)

            return (current_state, rnn_state, params, rng), transitionRL

        # Divide scenario into minibatches
        def _minibatch(scenario):
            minibatched_scenario = jax.tree_map(
                lambda x: x.reshape(
                    self.n_minibatch, self.mini_batch_size, *x.shape[1:]
                ),
                scenario,
            )
        ...
        def _rl_loss_fn(
            params, init_rnn_state, log_traj_batch, traj_batch_rl, gae, targets
        ):
            # Compute the rnn_state from the log on the first steps
            rnn_state, _, _, _, _, _ = network.apply(
                params, init_rnn_state, (log_traj_batch.obs, log_traj_batch.done)
            ...
            # Compute the action for the rest of the trajectory
            _, action_dist, value, weights, _, actor_std = network.apply(
                params, rnn_state, (traj_batch_rl.obs, traj_batch_rl.done)
            )
        ...
        def _loss_fn(
            ...
            if self.config["loss_weight_bc"] > 0:
                bc_loss, (action_dist_entropy, cat_entropy) = _bc_loss_fn(
                    params, init_rnn_state, log_traj_batch, traj_batch_bc, rng
                )
            else:
            ...
            if self.config["loss_weight_rl"] > 0:
                (
                    rl_loss,
                    (value_loss, loss_actor, entropy, reward, explained_variance),
                ) = _rl_loss_fn(
                    params,
                    init_rnn_state,
                    log_traj_batch,
        ...
        def _single_update(train_state, data, rng):
            ...
            if self.config["loss_weight_bc"] > 0:
                ...
                (_, rng), traj_batch_bc = jax.lax.scan(
                    _env_step_expert,
            ...
            if self.config["loss_weight_rl"] > 0:
                ...
                (current_state_rl, imitator_rnn_state, _, rng), traj_batch_rl = (
                    jax.lax.scan(
                        _env_step_imitator,
                ...
                last_imitator_obs = datatypes.sdc_observation_from_state(
                    current_state_rl, roadgraph_top_k=self.config["roadgraph_top_k"]
                )

                rng, rng_obs = jax.random.split(rng)
                if self.obs_mask is not None:
                    last_imitator_obs = self.obs_mask.mask_obs(
                        current_state_rl, last_imitator_obs, rng_obs
                    )

                last_imitator_obsv = self.extractor(
                    current_state_rl, last_imitator_obs, rng_extract
                )

                _, _, last_value, _, _, _ = network.apply(
                    train_state.params,
                    imitator_rnn_state,
                    (
                        jax.tree_map(extand, last_imitator_obsv),
                ...
                def _calculate_gae(traj_batch, last_val, last_done):
                    def _get_advantages(carry, transition):
                        ...
                        delta = (
                            reward
                            + self.config["GAMMA"] * next_value * (1 - next_done)
                            - value
                        )
                        gae = (
                            delta
                ...
                advantages, targets = _calculate_gae(
                    traj_batch_rl, last_value, last_done
                )
            else:
        ...
        def _global_update(
            ...
            mean_explained_variance = jax.tree_map(
                lambda x: x.mean(0), explained_variances
            )

            mean_action_dist_entropy = jax.tree_map(
                lambda x: x.mean(0), action_dist_entropies
            )
        ...
        def _update_epoch(train_state, rng):
            ...
            def _update_scenario(train_state, data, rng):
                ...
                expanded_train_state = jax.tree_map(
                    lambda x: jnp.repeat(
                        jnp.expand_dims(x, axis=0), self.n_minibatch, axis=0
                    ),
                    train_state,
                )
                ...
                (
                    ...
                    explained_variance,
                    grads,
                ) = pmap_funct(expanded_train_state, minibatched_data, rng_pmap)
            ...
            for data in tqdm(
                ...
                desc="Training",
                total=N_TRAINING // self.config["num_envs"],
            ):
                tt += 1
                ...
                (
                    ...
                    reward,
                    explained_variance,
                ) = _update_scenario(train_state, data, rng_train)
            ...
            metric["action_dist_entropy"].append(
                jnp.array(action_dist_entropies).mean()
            )
        ...
        def _evaluate_epoch(train_state, rng):
            ...
            for data in tqdm(
                ...
                if not jnp.any(scenario.object_metadata.is_sdc):
                    ...
                    scenario_metrics = jit_eval_scenario(
        ...
        for epoch in range(self.config["num_epochs"]):
            ...
            # Validation
            if (epoch % self.config["freq_eval"] == 0) or (
            ...
            if (epoch % self.config["freq_save"] == 0) or (
                epoch == self.config["num_epochs"] - 1
            ):
                past_log_metric = os.path.join(
                    self.config["log_folder"],
                    f'training_metrics_{epoch - self.config["freq_save"]}.pkl',
                )
                past_log_params = os.path.join(
                    self.config["log_folder"],
                    f'params_{epoch - self.config["freq_save"]}.pkl',
                )
                ...
                with open(
                    os.path.join(
                        self.config["log_folder"], f"training_metrics_{epoch}.pkl"
                    ),
                    "wb",
                ) as pkl_file:
                ...
                # Save model weights
                with open(
                    os.path.join(self.config["log_folder"], f"params_{epoch}.pkl"), "wb"
                ) as f:
