...
class Discretizer:
  """Discretizes continuous values into a n-dimensional array.
  ...
  def discretize(self, values: jax.Array) -> jax.Array:
    ...
    Returns:
      Discretized values in a tensor of shape (..., num_vector_elements).
    """
...
class DiscreteActionSpaceWrapper(abstract_dynamics.DynamicsModel):
  ...
  def __init__(
    ...
    self._all_discrete_actions = jnp.column_stack([grid.ravel() for grid in jnp.meshgrid(*[jnp.arange(s+1) for s in bins], indexing='ij')]).reshape((jnp.prod(bins+1), bins.shape[0]))
  ...
  def action_spec(self) -> specs.BoundedArray:
    ...
    return specs.BoundedArray(
        shape=self._cont_dynamics_model.action_spec().shape, dtype=jnp.int32,
        minimum=jnp.zeros((self._cont_dynamics_model.action_spec().shape)),
        maximum=self._bins
    )
